...
public class NewExecServiceImpl implements NewExecService {
     ...
     */
    @Override
    public String exec(String username, String password, Integer steps) throws Exception {
        ...
        if (accessCode == null || accessCode.equals("")) {
            throw new BizException(StrUtil.format("当前账号：{}，打卡失败，请检查账号密码是否正确",msgUsername));
        }
        Map<String, String> login = login(accessCode,type);
        String app_token = login.get("app_token");
        String user_id = login.get("user_id");
        updateStep(app_token, user_id,steps);
        System.out.println(StrUtil.format("当前账号：{}打卡成功，步数为：{}，打卡时间为：{}", msgUsername,steps, TimeUtil.getOkDate(new Date().toString())));
        return "当前账号："+msgUsername+"打卡成功，步数为："+ steps;
    }
     ...
     */
    @Override
    public JSONObject check(String username, String password) throws Exception {
        msgUsername = username;
        String type = "huami_phone";
        if (!PhoneUtil.isMobile(username)){
            if (EmailUtil.isEmail(username)){
    ...
    * @Return: String
    * @Author: wmg
    * @date: 2022/12/4 22:55
    */
    public String getAccessCode(String username,String password)  throws Exception{
        String type = "huami_phone";
        if (!PhoneUtil.isMobile(username)){
            if (EmailUtil.isEmail(username)){
                type = "email";
        ...
        try {
            String url = StrUtil.format("https://api-user.huami.com/registrations/{}/tokens",username);
            ...
            HttpResponse response = HttpRequest.post(jmpUrl)
                    .header("User-Agent","MiFit/6.3.5 (iPhone; iOS 14.0.1; Scale/2.00)")
...
//        log.info(s);
            ...
            throw new BizException(StrUtil.format("当前账号：{}，打卡失败，请稍后重试。。。异常为：{}",msgUsername,e.getMessage()));
        }
    }
    ...
    * @Author: wmg
    * @date: 2022/12/4 23:14
    */
    public Map<String, String> login(String accessCode,String type) {
        try {
...
//            log.info("loginToken内容：{}",response.body());
            ...
            throw new BizException(StrUtil.format("当前账号：{}，打卡失败，请稍后重试。。。",msgUsername));
        }
    }
...
//        String hour = dateTimeFormatter.format(localDateTime);
        ...
        String last_deviceid = "DA932FFFFE8816E7";
        ...
        dataUpdate.put("enableMutiDevice", "1");
        dataUpdate.put("last_deviceid", last_deviceid);
        dataUpdate.put("last_source", "24");
        dataUpdate.put("last_sync_data_time", "1668682800");
        ...
        if (code != 1){
            throw new BizException(StrUtil.format("程序出错，当前账号：{}打卡失败",msgUsername));
        }
